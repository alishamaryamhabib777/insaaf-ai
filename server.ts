...
export const ChatView: React.FC<ChatViewProps> = ({ language }) => {
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    try {
      // Map existing chat history for server payload
      const historyPayload = messages.map(msg => ({
        role: msg.sender === 'user' ? 'user' : 'model',
        content: msg.text
      }));

      // Post request to express chatbot endpoint (/api/chat)
      ...
      // Handle text output from server responses
      const responseText = data.text || data.caseSummary || "No response generated.";
      const legalBasis = data.legalBasis || 'Pakistan Statutory Framework (PPC, CrPC & SCMR)';
                ...
                <div className="space-y-2 pt-1">
                  <div className="p-3.5 bg-amber-50 rounded-2xl border border-amber-300 text-[11px] text-[#012618]">
                    ...
                    <p className="text-slate-700 text-[10px] font-medium leading-normal">
                      Cross-referenced with Official Gazette of Pakistan & Supreme Court Case Law Archives.
                    </p>

                    {/* Grounded Web Sources if available */}
                    {msg.sources && msg.sources.length > 0 && (
                      <div className="mt-2.5 pt-2 border-t border-amber-200 space-y-1">
                        <span className="text-[10px] font-bold text-amber-900 block">Verified Research Sources:</span>
                        <div className="flex flex-wrap gap-2">
                          {msg.sources.map((src: any, i: number) => (
                            <a
                              ...
                              className="text-[10px] text-emerald-800 underline hover:text-emerald-950 flex items-center space-x-1 font-semibold"
                            >
                              <span>{src.title}</span>
